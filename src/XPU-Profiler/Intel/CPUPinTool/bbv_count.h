...
class BbvCount
{
   ...
   public:
      ...
      int operator==(const BbvCount other) const
      {
          const auto& other_bbv_counts = other.getBbv();
          if (m_bbv_counts.size() != other_bbv_counts.size()) return 0;

          for (size_t i = 0 ; i < m_bbv_counts.size() ; i++)
              if (m_bbv_counts[i] != other_bbv_counts[i]) return 0;
